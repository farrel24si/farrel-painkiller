...
export const usersAPI = {
  // 1. Fungsi untuk Register User Baru
  async registerUser(userData) {
    const response = await axios.post(API_URL, userData, { headers });
    return response.data;
  },

  // 2. Fungsi untuk Login (Mencocokkan email & password)
  async loginUser(email, password) {
    // Kita filter data di Supabase yang email dan passwordnya cocok
    const response = await axios.get(
      `${API_URL}?email=eq.${email}&password=eq.${password}&select=*`, 
      { headers }
    );
    return response.data; // Akan mengembalikan array kosong [] jika gagal login
  },

  // 3. Fungsi untuk Mengambil Semua User (Untuk Halaman Admin Nanti)
  async fetchUsers() {
